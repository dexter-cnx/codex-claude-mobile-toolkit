---
title: "10. การออกแบบ Notification System"
aliases:
  - "Design A Notification System"
  - "การออกแบบ Notification System"
tags:
  - system-design
  - interview
  - distributed-systems
  - toolkit
  - obsidian
status: evergreen
type: note
chapter: 10
---

# 10. การออกแบบ Notification System

> [!summary]
> บทนี้อธิบาย **Design A Notification System** ในมุมของการออกแบบระบบ โดยเน้นว่า **Notification เป็น shared platform สำคัญของ product จริง ทั้ง push, email, SMS และ in-app**

## แก่นของบทนี้
- event-driven pipeline
- template rendering
- channel routing
- user preferences
- retry + DLQ
- idempotency

## ใช้เมื่อไร
- เมื่อโจทย์เกี่ยวกับการ scale ระบบหรือจัดการโหลดในระดับมาก
- เมื่อต้องอธิบาย trade-off ให้ชัดในเชิงสถาปัตยกรรม
- เมื่อต้องเตรียมตอบโจทย์สัมภาษณ์หรือเขียน design note ให้ทีม

## องค์ประกอบสำคัญ
- Event producer
- Notification orchestrator
- Preference store
- Template service
- Queue
- Channel adapters
- Tracking

## วิธีคิดแบบ interview-friendly
1. ระบุเป้าหมายของระบบก่อน
2. ประมาณ scale แบบคร่าว ๆ
3. วาด high-level components
4. ระบุจุดเสี่ยงหรือ bottleneck
5. อธิบาย trade-off ที่เลือก
6. สรุปสิ่งที่จะทำต่อถ้าระบบโตขึ้นอีก

## Trade-offs ที่ต้องพูดให้ได้
- ส่งทันทีเร็วแต่พึ่ง provider สูง
- batch ช่วยลด cost แต่ latency สูงขึ้น
- multi-channel เพิ่ม reach แต่ซับซ้อน

## ข้อผิดพลาดที่พบบ่อย
- duplicate send
- preference mismatch
- provider throttling
- lack of observability
- silent failures

## มุมมองสำหรับ Flutter / Mobile / Toolkit
- สำคัญมากกับ Flutter app ทุกประเภท โดยเฉพาะ push, in-app inbox, realtime reminders
- ใช้บทนี้เป็นฐานสำหรับเขียน prompt หรือ design review ใน `codex-claude-mobile-toolkit`
- ถ้าจะใช้กับ agent workflow ให้บังคับ agent สรุป assumption และ bottleneck ทุกครั้ง

## Prompt สั้นสำหรับ agent
```md
ออกแบบระบบหัวข้อ "Design A Notification System" โดยตอบตามลำดับนี้:
1. Functional requirements
2. Non-functional requirements
3. Capacity estimation
4. High-level architecture
5. Data model / storage choices
6. APIs หรือ event flow
7. Bottlenecks
8. Trade-offs
9. Evolution path เมื่อระบบโตขึ้น
```

## โน้ตเชื่อมโยง
- [[00-system-design-home]]
- [[99-system-design-glossary]]
- [[09-web-crawler]]
- [[11-news-feed-system]]

## Checklist ทบทวนตัวเอง
- [ ] อธิบายโจทย์นี้ได้ใน 2-3 นาที
- [ ] รู้ component หลักของระบบ
- [ ] รู้ trade-off อย่างน้อย 2 แบบ
- [ ] รู้ bottleneck ที่น่าจะเจอ
- [ ] รู้ว่าถ้า scale โตขึ้นจะ evolve อย่างไร
